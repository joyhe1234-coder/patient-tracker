        ...
        <div className="relative bg-white rounded-lg shadow-xl max-w-lg w-full p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-lg font-semibold text-gray-900">Assign Patients</h3>
                <p className="text-sm text-gray-500">
                  {totalCount.toLocaleString()} patient{totalCount !== 1 ? 's' : ''} selected
                </p>
              </div>
            </div>
          ...
          {/* Physician Dropdown */}
          <div className="mb-4">
            <label htmlFor="physician-select" className="block text-sm font-medium text-gray-700 mb-1">
              Assign to Physician
            </label>
            <select
              id="physician-select"
              value={selectedPhysicianId ?? ''}
              onChange={(e) => setSelectedPhysicianId(e.target.value ? Number(e.target.value) : null)}
          ...
          {/* Patient Preview */}
          <div className="mb-4">
            <p className="text-sm font-medium text-gray-700 mb-2">Affected patients:</p>
              ...
              {previewPatients.map((patient) => (
                ...
                >
                  ...
                  <span className="text-gray-500 ml-2">
                    {patient.ownerName ? `(${patient.ownerName})` : '(Unassigned)'}
                  </span>
                </div>
              ))}
            </div>
          ...
          <div className="flex justify-end gap-3">
            ...
            <button
              onClick={() => selectedPhysicianId && onConfirm(selectedPhysicianId)}
              disabled={!selectedPhysicianId || loading}
